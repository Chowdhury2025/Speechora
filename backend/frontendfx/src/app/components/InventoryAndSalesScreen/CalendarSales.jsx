...
const CalendarWithPopupTable = ({ userState }) => {
  ...
  // Fetch stores on component mount and handle permissions
  useEffect(() => {
    const fetchStores = async () => {
      try {
        const res = await fetch(`${API_URL}/api/calendar/stores`);
        ...
        if (canViewAllStores) {
          ...
          const userStores = data.stores.filter(
            store => userState.storeIds.includes(store.id)
          );
          setStores(userStores);
          
          // Auto-select the store if user has only one
          if (userStores.length === 1) {
            setStoreFilter(userStores[0].id.toString());
          }
        }
      } catch (err) {
        console.error('Error fetching stores for calendar:', err);
    ...
    fetchStores();
  }, [API_URL, canViewAllStores, userState.storeIds]);
