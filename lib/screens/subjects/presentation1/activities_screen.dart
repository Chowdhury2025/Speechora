...
class ActivitiesScreen extends StatefulWidget {
  ...
  const ActivitiesScreen({super.key, required this.backgroundColor});
  ...
  Future<void> fetchImages() async {
    try {
      final response = await http.get(
        Uri.parse('${Constants.baseUrl}/images?category=activities'),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data.isEmpty) {
          if (mounted) {
            setState(() {
              isLoading = false;
              error = 'No activity images available';
            });
          }
          return;
        }
